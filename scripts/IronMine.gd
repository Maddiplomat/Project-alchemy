...
func _do_mine() -> void:
	...
	InventoryManager.add_element("iron", yield_amount, 1.0)
...
func _build_visuals() -> void:
	var ore_shapes: Array[Array] = [
		[Vector2(-16, 5), [Vector2(-13,-10), Vector2(2,-15), Vector2(13,-8), Vector2(10,6), Vector2(-6,12), Vector2(-15,2)], Color(0.40, 0.27, 0.23)],
		[Vector2(14, 0), [Vector2(-9,-9), Vector2(5,-13), Vector2(13,-2), Vector2(9,11), Vector2(-7,8)], Color(0.33, 0.24, 0.22)],
		[Vector2(-2, 11), [Vector2(-11,-6), Vector2(8,-9), Vector2(13,4), Vector2(3,11), Vector2(-12,7)], Color(0.47, 0.33, 0.28)],
		[Vector2(1, -14), [Vector2(-7,-6), Vector2(7,-9), Vector2(10,2), Vector2(4,9), Vector2(-8,5)], Color(0.55, 0.41, 0.34)],
	]
	...
	seam1.default_color = Color(0.78, 0.46, 0.30, 0.9)
	seam1.width = 1.4
	seam1.z_index = 2
	...
	seam2.default_color = Color(0.68, 0.38, 0.24, 0.82)
	seam2.width = 1.2
	seam2.z_index = 2
...
func _refresh_label() -> void:
	if _label == null:
		return
	if _stock <= 0:
		_label.text = "Iron Mine\n[Depleted]"
		_label.add_theme_color_override("font_color", Color(0.78, 0.46, 0.40))
	else:
		_label.text = "Iron Mine  (%d)\n[Tap Attack] Mine" % _stock
		_label.add_theme_color_override("font_color", Color(0.96, 0.86, 0.77))
...
func _flash_rocks() -> void:
	for rock in _rocks:
		if rock is Polygon2D:
			...
			rock.color = Color(0.86, 0.58, 0.43)
