...
func _ready() -> void:
	if MapMarkers != null:
		MapMarkers.markers_changed.connect(queue_redraw)
...
func _draw() -> void:
	...
	var world_rect := MapMarkers.get_world_rect()
	if world_rect.size.x <= 0.0 or world_rect.size.y <= 0.0:
		return

	for marker: Dictionary in MapMarkers.get_markers():
	...
	var player_position: Vector2 = MapMarkers.get_player_world_position()
